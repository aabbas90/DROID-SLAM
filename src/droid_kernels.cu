...
public:
  ...
  std::tuple<torch::Tensor, torch::Tensor> get_dense()
  {
    ...
    torch::Tensor H = torch::from_blob(Ad.data(), {N * M, N * M}, torch::TensorOptions().dtype(torch::kFloat64)).to(torch::kCUDA).to(torch::kFloat32);

    torch::Tensor v = torch::from_blob(b.data(), {N * M, 1}, torch::TensorOptions().dtype(torch::kFloat64)).to(torch::kCUDA).to(torch::kFloat32);
  ...
  torch::Tensor solve(const float lm = 0.0001, const float ep = 0.1)
  {
    ...
    if (solver.info() == Eigen::Success)
    {
      ...
      dx = torch::from_blob(x.data(), {N, M}, torch::TensorOptions().dtype(torch::kFloat64)).to(torch::kCUDA).to(torch::kFloat32);
    }
    else
    {
